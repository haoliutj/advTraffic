...
shs: smart home speaker traffic analysis
"""
...
class train_target_model:
    def __init__(self,opts):
        ...
        self.opts = opts
        self.mode = opts['mode']
        self.model_path = '../model/' + self.mode + '/cnn'
    ...
    def train_model(self):
        ...
        if self.mode in [ 'wf', 'detect', 'wf_ow','wf_kf']:
            ...
            print('mode not in ["wf","shs","wf_ow","detect","wf-kf"], system will exit.')
            sys.exit()
...
def main(opts):
    ...
    print('training time is %.2f seconds.' % run_time)
...
def get_opts_wf_ow(mode):
    return{
        ...
        'test_data_path': '../data/wf_ow/test_NoDef_UnMon.csv',
    }
