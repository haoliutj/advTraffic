...
class cnn_noNorm(nn.Module):
    ...
    def __init__(self,params):
        ...
        # https://blog.csdn.net/cz505632696/article/details/86179811
        # 在forward里 self.flatten = torch.flatten()
...
class DF_CNN(nn.Module):
    """
    DF CNN model in Pytorch from "Deep Fingerprinting: Undermining Website Fingerprinting Defenses with Deep Learning"
    P. Sirinam, et al., 2018

    note: the input_features of first FC layer is obtained manually by only forward block 1-4 and see the output shape
    """
    def __init__(self,num_class=95):
        super(DF_CNN,self).__init__()

        self.block1 = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=32,kernel_size=8, stride=1, padding=3),
            nn.BatchNorm1d(32),
            nn.ELU(alpha=1.0),
        ...
        self.block2 = nn.Sequential(
            nn.Conv1d(in_channels=32, out_channels=64, kernel_size=8, stride=1, padding=9),
            nn.BatchNorm1d(64),
            nn.ReLU(),
            nn.Conv1d(in_channels=64, out_channels=64, kernel_size=8, stride=1, padding=9),
            nn.BatchNorm1d(64),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=8,stride=4),
            nn.Dropout(0.1)
        ...
        self.block3 = nn.Sequential(
            nn.Conv1d(in_channels=64, out_channels=128, kernel_size=8, stride=1, padding=6),
            nn.BatchNorm1d(128),
            nn.ReLU(),
            nn.Conv1d(in_channels=128, out_channels=128, kernel_size=8, stride=1, padding=6),
            nn.BatchNorm1d(128),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=8,stride=4),
            nn.Dropout(0.1)
        ...
        self.block4 = nn.Sequential(
            nn.Conv1d(in_channels=128, out_channels=256, kernel_size=8, stride=1, padding=11),
            nn.BatchNorm1d(256),
            nn.ReLU(),
            nn.Conv1d(in_channels=256, out_channels=256, kernel_size=8, stride=1, padding=11),
            nn.BatchNorm1d(256),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=8,stride=4),
            nn.Dropout(0.1)
        ...
        self.fc = nn.Sequential(
            nn.Flatten(),
            nn.Linear(256*9,512),   #256*9 is obtained manually based on the output of above 4 blocks
            nn.BatchNorm1d(512),
            nn.ReLU(),
            nn.Dropout(0.7),
            nn.Linear(512,512),
        ...
        self.dense = nn.Linear(512,num_class)      # multi-classification 95 classes
        self.softmax = nn.Softmax(dim=1)    # dim=1 --> Row; dim=0 --> Col
    ...
    def forward(self,x):
        ...
        x = self.fc(x)
        logits = self.dense(x)
        conf_score = self.softmax(logits)
        return conf_score


#---------------------------------------------
class DF_CNN_ow(nn.Module):
    """
    parameter tuned of DF-CNN model for Open-world setting since imbalance data here
    DF CNN model in Pytorch from "Deep Fingerprinting: Undermining Website Fingerprinting Defenses with Deep Learning"
    P. Sirinam, et al., 2018
    ...
    def __init__(self,num_class=95):
        ...
        self.block2 = nn.Sequential(
            nn.Conv1d(in_channels=32, out_channels=64, kernel_size=19, stride=1, padding=9),
            nn.BatchNorm1d(64),
            nn.ReLU(),
            nn.Conv1d(in_channels=64, out_channels=64, kernel_size=19, stride=1, padding=9),
            nn.BatchNorm1d(64),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2),
            nn.Dropout(0.1)
        ...
        self.block3 = nn.Sequential(
            nn.Conv1d(in_channels=64, out_channels=128, kernel_size=13, stride=1, padding=6),
            nn.BatchNorm1d(128),
            nn.ReLU(),
            nn.Conv1d(in_channels=128, out_channels=128, kernel_size=13, stride=1, padding=6),
            nn.BatchNorm1d(128),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2),
            nn.Dropout(0.3)
        ...
        self.block4 = nn.Sequential(
            nn.Conv1d(in_channels=128, out_channels=256, kernel_size=23, stride=1, padding=11),
            nn.BatchNorm1d(256),
            nn.ReLU(),
            nn.Conv1d(in_channels=256, out_channels=256, kernel_size=23, stride=1, padding=11),
            nn.BatchNorm1d(256),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2),
            nn.Dropout(0.1)
        ...
        self.fc = nn.Sequential(
            nn.Flatten(),
            nn.Linear(256*32,512),   #256*4 is obtained manually based on the output of above 4 blocks
            nn.BatchNorm1d(512),
            nn.ReLU(),
            nn.Dropout(0.7),
            nn.Linear(512,512),
