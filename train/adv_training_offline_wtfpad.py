...
class adv_train_deepfool:
    ...
    def adv_train_process(self,delay=0.5):
        ...
        elif self.opts['Adversary'] == 'WTFPAD':
            adv_train_data_path = '../data/wf/DF-CNN/DF-CNN_source_data/adv_train_WTFPAD.csv'
        else:
        ...
        if self.mode in ['wf','wf_ow','wf_kf']:
            ...
            elif self.classifier_type == 'DF-CNN'or'DF-CNN/DF-CNN_source_data':
                target_model = models.DF_CNN().to(self.device)
                target_model.train()
...
def get_opts_wf(mode,Adversary,classifier_type):
    ...
    return {
        ...
        'input_size':512,
        'num_class':95,
        'epochs': 5,  # for models built ourself its 50, DF-CNN:200
        'batch_size': 64,
        # 'train_data_path': 'train_NoDef_burst.csv', # without splitting into validation
        'train_data_path': 'NoDef/csv_source_data/train_NoDef.csv', # after splitting into validation
        'test_data_path': 'NoDef/csv_source_data/test_NoDef.csv',
...
if __name__ == '__main__':
    ...
    Adversary = ['WTFPAD']                             # ['FGSM','DeepFool','PGD','GAN',WT','Mockingbird']
    classifier_type = 'DF-CNN/DF-CNN_source_data/'                          # ['lstm','cnn','DF-CNN']
    k = 5 # K-fold
